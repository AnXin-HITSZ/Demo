# 案例 1：定义一个函数，根据传入的底和高计算三角形面积（三角形面积 = 底 * 高 / 2）
...
print("底长为 30、高为 20 的三角形面积：", triangle_area(30, 20))

# 案例 2：定义一个函数，计算传入的字符串中元音字母的个数（元音字母为 a、e、i、o、u、A、E、I、O、U）
def count_aeiou(s):
    """
    统计字符串中元音字母的个数
    :param s: 字符串
...
# 案例 3：定义一个函数，计算传入的班级学员高考成绩列表中成绩的最高分、最低分、平均分（保留 1 位小数），并返回
def calc_score(score_list):
    """
    计算传入的班级学员高考成绩列表中成绩的最高分、最低分、平均分（保留 1 位小数）
